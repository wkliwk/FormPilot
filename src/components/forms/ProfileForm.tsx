...
export default function ProfileForm({ initialData, initialPreferredLanguage }: Props) {
  ...
  async function handleExportProfile() {
    ...
    setExportError(null);
    try {
      const res = await fetch("/api/profile/export-token", { method: "POST" });
      const data = await res.json();
      if (!res.ok) {
        setExportError(data.error ?? "Could not generate export link.");
        setExportLoading(false);
        return;
      }
      const qrDataUrl = await QRCode.toDataURL(data.importUrl, { width: 200, margin: 1 });
      setExportUrl(data.importUrl);
      setExportQr(qrDataUrl);
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="border-t border-slate-100 pt-4 space-y-2">
        <p className="text-xs text-slate-500">Share your profile with another FormPilot account (sensitive fields excluded).</p>
        {exportError && (
        ...
        <button
          ...
          className="w-full inline-flex items-center justify-center gap-2 py-2.5 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
          {exportLoading ? (
            <>
              <svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none" aria-hidden="true">
    ...
    {exportOpen && (
      ...
      >
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-5">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-semibold text-slate-900">Share Profile</h2>
            <button
              type="button"
          ...
          {exportQr && (
            <div className="flex justify-center">
              ...
              <img src={exportQr} alt="Profile import QR code" className="w-40 h-40 rounded-xl border border-slate-100" />
            </div>
          )}

          <div className="space-y-1">
            <p className="text-xs text-slate-500">Scan the QR code or copy the link. It expires in 15 minutes.</p>
            <div className="flex items-center gap-2">
              <input
                type="text"
                readOnly
                value={exportUrl}
                className="flex-1 text-xs px-3 py-2 border border-slate-200 rounded-lg bg-slate-50 text-slate-700 truncate focus:outline-none"
              />
              <button
                type="button"
                onClick={handleCopy}
          ...
          {exportFields.length > 0 && (
            <div className="space-y-1">
              <p className="text-xs font-medium text-slate-600">Included fields:</p>
          ...
          <p className="text-xs text-slate-400 flex items-start gap-1.5">
            ...
            Sensitive fields (SSN, passport, driver&apos;s license) are never included.
          </p>
        </div>
      </div>
    )}
